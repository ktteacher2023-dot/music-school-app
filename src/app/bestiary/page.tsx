    ...
    <div className="min-h-screen" style={{ background: theme.bgPage }}>
      <header className="bg-white/85 backdrop-blur-xl sticky top-0 z-10 border-b border-[#C6C6C8]/60"
        ...
        <div className="px-4 py-3">
          <h1 className="text-2xl font-bold text-[#1C1C1E]">{theme.encyclopediaTitle}</h1>
          <p className="text-xs mt-0.5" style={{ color: title.color }}>
            {title.icon} {title.title} · {isPrincess ? 'なかよし' : '登録'} {defeatedIds.length} / {creatures.length} 体
          </p>
        </div>
      </header>
          ...
          <div className="bg-white rounded-2xl px-4 py-3 shadow-sm mb-4">
            ...
            <div className="h-2.5 bg-[#F2F2F7] rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-700"
                style={{ width:`${(defeatedIds.length/creatures.length)*100}%`,
                  background: isPrincess
                    ? 'linear-gradient(90deg,#FF6B9D,#C77DFF)'
                    : 'linear-gradient(90deg,#34C759,#007AFF)' }}/>
            </div>
            {defeatedIds.length === creatures.length && (
              <p className="text-center text-xs font-bold mt-1.5"
                style={{ color: isPrincess ? '#FF6B9D' : '#34C759' }}>
