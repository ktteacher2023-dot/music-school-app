...
const BASE_EXP  = [2, 4, 8, 15, 30, 50];
const HARD_BONUS = 5; // per hard question correct
...
function SymbolDisplay({ symbol, symbolSize, isPrincess }: { symbol: string; symbolSize: number; isPrincess: boolean }) {
  return (
    <div style={{
      ...
      <span style={{
        fontSize: symbolSize, fontFamily: 'Georgia, "Times New Roman", serif',
        fontStyle: /^[a-z]/.test(symbol) ? 'italic' : 'normal',
        color: isPrincess ? '#C77DFF' : '#FF9F0A', fontWeight: 900,
        textShadow: isPrincess ? '0 0 24px rgba(199,125,255,0.9)' : '0 0 24px rgba(255,159,10,0.9)',
        lineHeight: 1,
      }}>
        {symbol}
      </span>
...
// ─── Difficulty stars ────────────────────────────────────────────────────────
function DiffBadge({ diff, isPrincess }: { diff: number; isPrincess: boolean }) {
  ...
  return (
    <span style={{
      ...
      background: hard
        ? (isPrincess ? 'rgba(255,59,48,0.18)' : 'rgba(255,59,48,0.22)')
        : (isPrincess ? 'rgba(255,107,157,0.18)' : 'rgba(255,159,10,0.18)'),
      color: hard ? '#FF3B30' : (isPrincess ? '#FF6B9D' : '#FF9F0A'),
      border: `1px solid ${hard ? '#FF3B3055' : (isPrincess ? '#FF6B9D55' : '#FF9F0A55')}`,
      letterSpacing: '0.1em',
    }}>
      {hard ? '★★★ HARD' : '★★ NORMAL'}
    </span>
...
export default function MusicQuizGame({ isPrincess, onGameEnd }: Props) {
  ...
  const [items,          setItems]          = useState<{ question: MusicQuestion; isReview: boolean }[]>([]);
  ...
  const [gameResults,    setGameResults]    = useState<{ id: string; correct: boolean }[]>([]);
  const [finalScore,     setFinalScore]     = useState(0);
  const [finalHard,      setFinalHard]      = useState(0);
  const [leveledUp,      setLeveledUp]      = useState(false);
  const [newQuizLevel,   setNewQuizLevel]   = useState(1);
  ...
  const startGame = () => {
    ...
    setItems(finalItems);
    setIdx(0); setScore(0); setFinalScore(0); setHardCorrect(0); setFinalHard(0);
    setGameResults([]); setSelectedIdx(null); setLeveledUp(false);
    setPhase('question');
  };
  ...
  const handleChoice = (choiceIdx: number) => {
    ...
    const isHard  = q.difficulty >= 4;
    const newScore     = correct ? score + 1 : score;
    const newHardCount = (correct && isHard) ? hardCorrect + 1 : hardCorrect;
    const newResults   = [...gameResults, { id: q.id, correct }];

    setLastCorrect(correct); setSelectedIdx(choiceIdx);
    setScore(newScore); setHardCorrect(newHardCount); setGameResults(newResults);

    if (correct) {
      setAnimKey(k => k + 1);
      if (!isPrincess) { setShowInk(true);     setTimeout(() => setShowInk(false), 900); }
      else             { setShowSparkle(true);  setTimeout(() => setShowSparkle(false), 1200); }
      if (isHard) { setCriticalHit(true); setTimeout(() => setCriticalHit(false), 1600); }
    }

    setPhase('feedback');
    const isLast = idx + 1 >= items.length;
    const delay  = (correct && isHard) ? 2600 : 2200;
    setTimeout(() => {
      if (isLast) {
        // Finalise progress
        const progress = loadProgress();
        const updated  = finishGame(progress, newResults, newScore, items.length);
        saveProgress(updated);
        setLeveledUp(updated.level > progress.level);
        setNewQuizLevel(updated.level);
        setFinalScore(newScore);
        setFinalHard(newHardCount);
        setPhase('result');
      } else {
        setIdx(i => i + 1); setSelectedIdx(null); setCriticalHit(false);
        setPhase('question');
      }
    }, delay);
  };
  ...
  if (phase === 'intro') {
    const progress = loadProgress();
    const weak = getWeakIds(progress);
    const reviewCount = Math.min(2, [...new Set([...progress.lastWrongIds, ...weak])].length);
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-5" style={bgStyle}>
        <div style={{ fontSize: 56, marginBottom: 8, filter: `drop-shadow(0 0 12px ${accent}aa)` }}>
          {isPrincess ? '🎼' : '🎵'}
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: textMain, textShadow: `0 0 20px ${accent}88`, letterSpacing: '0.06em', marginBottom: 4, textAlign: 'center' }}>
          {isPrincess ? '音楽パズル' : '音撃クイズ'}
        </h1>

        {/* Current quiz level badge */}
        <div style={{
  ...
  if ((phase === 'question' || phase === 'feedback') && q && currentItem) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col" style={bgStyle}>
        ...
        {/* Knight ink splash */}
        {showInk && (
          <div key={`ink-${animKey}`} style={{
            position: 'fixed', top: '50%', left: '50%', width: 60, height: 60, borderRadius: '50%',
        ...
        {/* CRITICAL HIT / MIRACLE MAGIC overlay */}
        {criticalHit && (
          <div style={{
            ...
            <div style={{
              fontSize: isPrincess ? 32 : 36, fontWeight: 900, letterSpacing: '0.06em',
              color: isPrincess ? '#FFD700' : '#FF9F0A',
              textShadow: isPrincess
                ? '0 0 30px rgba(255,215,0,0.9), 0 0 60px rgba(199,125,255,0.6)'
                : '0 0 30px rgba(255,107,0,0.9), 0 0 60px rgba(255,215,0,0.6)',
              animation: 'graffitiIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards',
              textAlign: 'center', lineHeight: 1.2,
            }}>
              {isPrincess ? '✨ MIRACLE MAGIC!' : '🔥 CRITICAL HIT!'}
              <div style={{ fontSize: 20, marginTop: 6 }}>
                {isPrincess ? '+5 ボーナスEXP！' : '+5 BONUS EXP!'}
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 20px 24px' }}>
          ...
          {currentItem.isReview && (
            <div style={{
              marginBottom: 10, padding: '6px 12px', borderRadius: 10, textAlign: 'center',
              background: isPrincess ? 'rgba(255,107,157,0.15)' : 'rgba(255,215,0,0.12)',
              border: isPrincess ? '1px solid rgba(255,107,157,0.4)' : '1px solid rgba(255,215,0,0.4)',
            }}>
              <span style={{ fontSize: 12, fontWeight: 900, color: isPrincess ? '#FF6B9D' : '#FFD700' }}>
                📖 復習チャンス！前回間違えた問題だよ
              </span>
            </div>
          )}
          ...
          <div style={{
            ...
            border: `1px solid ${accent}33`, padding: '16px 8px', marginBottom: 12,
          }}>
            {q.type === 'note'
          ...
          <p style={{ fontSize: 17, fontWeight: 900, color: textMain, textAlign: 'center', marginBottom: 16, lineHeight: 1.4 }}>
            {q.question}
          </p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-5" style={bgStyle}>
      ...
      {/* EXP */}
      <div style={{
        marginTop: 18, padding: '18px 32px', borderRadius: 20, textAlign: 'center',
        ...
        <p style={{ fontSize: 48, fontWeight: 900, lineHeight: 1, color: accent, textShadow: `0 0 24px ${accent}` }}>
          +{expGained}
        </p>
        {finalHard > 0 && (
          <p style={{ fontSize: 12, color: accent, marginTop: 4 }}>
            {isPrincess ? `✨ 難問ボーナス +${finalHard * HARD_BONUS}EXP含む！` : `🔥 難問ボーナス +${finalHard * HARD_BONUS}EXP含む！`}
          </p>
        )}
      </div>

      <button onClick={() => onGameEnd(expGained)} style={{
        marginTop: 20, width: '100%', maxWidth: 300, padding: '16px 0', borderRadius: 14,
